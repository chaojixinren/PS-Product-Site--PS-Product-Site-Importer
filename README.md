...
### 插件特点
* 使用 iframe 隔离渲染，默认避免主题CSS/JS冲突
* 图片回退机制：`ps_img1` → 特色图像 → `A5/A8/A11`
* 智能搜索覆盖标题、型号、描述、亮点、场景、参数表文本
* 参数表支持 HTML `<table>` 标签并可标签切换显示
* 兼容多种 REST 请求路径（`/wp-json/` 和 `?rest_route=`）

### 前端技术栈
* 原生 JavaScript 实现
* 响应式 CSS 设计
* iframe 自适应高度脚本（去抖 + ResizeObserver）
...
### 安全措施
* 所有输出均经过适当转义
* CSV 导入需确保 UTF-8 BOM 编码，防止乱码注入
* 使用 WordPress 原生函数处理数据和权限验证