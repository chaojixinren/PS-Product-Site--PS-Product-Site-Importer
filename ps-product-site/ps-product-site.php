    ...
    class PS_Product_Site_Plugin {
      ...
      function rest_products($req){
        ...
        $items=[]; while($q->have_posts()){ $q->the_post(); $id=get_the_ID();
          ...
          $items[]=[
            'id'=>$id,'title'=>get_the_title(),'sub'=>get_post_meta($id,'ps_sub',true),'desc'=>wp_strip_all_tags(get_the_content('',false)),
            'img1'=>$this->get_image_or_featured($id,'ps_img1'),'A5'=>$this->get_image_or_featured($id,'ps_img2'),'A8'=>$this->get_image_or_featured($id,'ps_img3'),'A11'=>$this->get_image_or_featured($id,'ps_img4'),
