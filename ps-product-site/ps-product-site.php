 ...
 * Description: v1.3.5：数据源更稳（自动回退至 ?rest_route= 形式），即使服务器不支持 /wp-json/ 重写也能加载。
 * Version: 1.3.5
 * Author: 超級の新人
 */
...
class PS_Product_Site_Plugin {
  ...
  function render_metabox($post){
    ...
    $m=['ps_sub'=>$this->field('ps_sub'),'ps_img1'=>$this->field('ps_img1'),'ps_img2'=>$this->field('ps_img2'),'ps_img3'=>$this->field('ps_img3'),'ps_img4'=>$this->field('ps_img4'),
    ...
    echo '<div class="ps-metabox"><table class="form-table">';
  ...
  private function inject_rewrite_js($endpoints, $id){
    $p = esc_js($endpoints['primary']);
    $f = esc_js($endpoints['fallback']);
    return '<script>(function(){var EP_PRIMARY=\"'.$p.'\";var EP_FALLBACK=\"'.$f.'\";var EP=EP_PRIMARY;function needsRewrite(u){return typeof u===\"string\" && u.indexOf(\"data.json\")!==-1;}var _fetch=window.fetch;if(_fetch){window.fetch=function(i,init){var orig=(typeof i===\"string\")?i:(i&&i.url?i.url:\"\");if(needsRewrite(orig)){return _fetch(EP,init).then(function(r){if(!r.ok){return _fetch(EP_FALLBACK,init);}return r;}).catch(function(){return _fetch(EP_FALLBACK,init);});}return _fetch(i,init);};}var XO=XMLHttpRequest&&XMLHttpRequest.prototype&&XMLHttpRequest.prototype.open;if(XO){XMLHttpRequest.prototype.open=function(m,u){if(needsRewrite(u)){arguments[1]=EP;}return XO.apply(this,arguments);};}if(window.axios&&window.axios.get){var OG=window.axios.get;window.axios.get=function(u,c){if(needsRewrite(u)){u=EP;}return OG.call(this,u,c);};}window.PS_PRODUCTS_ENDPOINT=EP;})();</script>';
  }
  ...
  function shortcode_catalog($atts=[]){
    ...
    $html=$this->inject_rewrite_js($eps, 'inline').$html;
    if($atts['fullwidth']==='1'){
      $max=intval($atts['maxwidth']); if($max<=0) $max=1280;
  ...
  function maybe_render_iframe(){
    if(isset($_GET['ps_catalog_iframe'])){
      ...
      echo '</head><body>';
      echo $this->inject_rewrite_js($eps, $id);
      echo $html;
